...
export default class DB extends BaseDB {
    ...
    public async getContactInfo(): Promise<ContactInfoResponse> {
        const results = await this.getDocumentById<ContactInfo>(PortfolioConstants.contactInfoDbCollectionName, PortfolioConstants.profileId);

        return new ContactInfoResponse(results);
    }

    public async getProfile(): Promise<ProfileResponse> {
        const results = await this.getDocumentById<Profile>(PortfolioConstants.profileDbCollectionName, PortfolioConstants.profileId);

        return new ProfileResponse(results);
    }

    public async getProjectById(id: string): Promise<ProjectResponse> {
        const results = await this.getDocumentById<Project>(PortfolioConstants.projectsDbCollectionName, id);

        return new ProjectResponse(results);
    }

    public async getProjects(): Promise<ProjectsResponse> {
        const results = await this.getCollectionByName<Project>(PortfolioConstants.projectsDbCollectionName);
